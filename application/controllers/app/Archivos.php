...
class Archivos extends CI_Controller {
	...
	public function addArchivoRegistro($id){
		$data = array('id_registro' => $id);
		$this->load->view("layouts/header");
		$this->load->view("layouts/aside");
		$this->load->view("app/archivos/registro/addArchivo",$data);
		$this->load->view("layouts/footer");
	}
	public function subirArchivoRegistro($id){
		$config['upload_path'] = './assets/uploads/registros/archivos';
		$config['allowed_types'] = 'pdf|xlsx|docx|doc';
		$config['max_size'] = '20048';
		if ($this->input->post("titulo_documento")==="") {
			$config['file_name'] = $this->input->post("id_registro")."-archivo";
		} else {
			$config['file_name'] = $this->input->post("id_registro")."-".$this->input->post("titulo_documento");
		}
		$this->load->library('upload',$config);
		...
		//	$data['error'] = "";
			...
			$datosArchivo = array(
				...
				//	$subir = $this->Archivos_model->subir($titulo,$imagen,$id);
			if ($this->Archivos_model->subirArchivoRegistro($datosArchivo)) {
					redirect(base_url()."app/registros/info/".$id);
			}else{
					$this->session->set_flashdata("error","No se pudo guardar la informacion");
				redirect(base_url()."app/archivos/addArchivoRegistro/".$id);
			}
			//$this->load->view('layout/header');
			//$this->load->view('layout/menu');
			//$this->load->view('vupload', $data);
	...
	public function downloadsArchivoRegistro($name){
		$data = file_get_contents('./assets/uploads/registros/archivos/'.$name);
	...
	public function deleteArchivoRegistro($id){
		$data  = array(
			'id_estatus' => "0",
		);
		$this->Archivos_model->delete($id,$data,'archivos_registro');
	...
	public function deleteArchivo($id){
		...
		$this->Archivos_model->delete($id,$data,'archivos_periodista');
		header('Location:' . getenv('HTTP_REFERER'));
	}
	public function deleteImagen($id){
		...
		$this->Archivos_model->deleteImagen($id,$data,'imagenes_periodista');
		//	redirect(base_url()."app/periodistas/edit/$id_periodista");
		header('Location:' . getenv('HTTP_REFERER'));
	}
	public function deleteImagenRegistro($id){


		$data  = array(
			'id_estatus' => "0",
		);
		$this->Archivos_model->deleteImagen($id,$data,'imagenes_registro');
		//	redirect(base_url()."app/periodistas/edit/$id_periodista");
		header('Location:' . getenv('HTTP_REFERER'));
	}
	public function addImagenRegistro($id){
		$data = array('id_registro' => $id);
		$this->load->view("layouts/header");
		$this->load->view("layouts/aside");
		$this->load->view("app/archivos/registro/addImagen",$data);
		$this->load->view("layouts/footer");
	}
	private function foto_registro_redimencionar($imagen) {
		$configthumbs['image_library'] = 'gd2';
	 	$configthumbs['source_image'] = './assets/uploads/registros/imagenes/'.$imagen;
	 	$configthumbs['new_image'] = './assets/uploads/registros/imagenes/thumbs/'.$imagen;
	 	$configthumbs['maintain_ratio'] = TRUE;
		$configthumbs['width'] = 30;
		$configthumbs['height'] =30;
		$this->load->library('image_lib', $configthumbs);
	...
	public function subirImagenRegistro($id) {
		$mi_archivo = 'archivo_imagen';
		$config['upload_path'] = "./assets/uploads/registros/imagenes/";
		if ($this->input->post("titulo_imagen")==="") {
			$config['file_name'] = $this->input->post("id_registro")."-imagen";
		} else {
			$config['file_name'] = $this->input->post("id_registro")."-".$this->input->post("titulo_imagen");
		}
		$config['allowed_types'] = 'gif|jpg|png';
		$config['max_size'] = "50000";
		$config['max_width'] = "2000";
		...
		$this->foto_registro_redimencionar($imagen);
		//	$this->foto_vistaprevia($imagen);
		$datosImagen = array(
			'titulo' => $titulo,
			'ruta' => $imagen,
			...
			//	$subir = $this->Archivos_model->subir($titulo,$imagen,$id);
		if ($this->Archivos_model->subirImagenRegistro($datosImagen)) {
				redirect(base_url()."app/registros/info/".$id);
		}else{
			$this->session->set_flashdata("error","No se pudo guardar la informacion");
			redirect(base_url()."app/archivos/add/".$id);
